...
class Comment {
    ...
    CommentModel.findOne({name: req.body.name, articleId: req.body.articleId}, (err, doc) => {
      ...
      if (doc && doc.address !== req.body.address) {
        ...
        if (/^@(.*):/.test(req.body.content)) {
          ...
          CommentModel.findOne({name: reviewer, articleId: req.body.articleId}, (err, doc) => {
            if (err) {
              console.log(err)
            }
            const url = 'https://www.xxx.cn' + req.body.curPath
            const replyEmail = doc.address
            const content = this.emailForm('欢迎常来我的博客', reviewer, req.body.name, '回复了你的评论', req.body.content, url)
            mail.send(replyEmail, '您在FatDong的博客有一条新评论', content, res)
          })
        }
        new CommentModel(comment).save().then(() => {
          const url = 'https://www.xxx.cn' + req.body.curPath
  ...
  emailForm (title, name, otherName, message, content, url) {
    let string = `
